...
public class ContentsItemBuilder {
  ...
  public HtmlTag buildItem(SourcePage page) {
    ...
    String approved = page.getProperty(WikiPageProperty.APPROVER);
    boolean isApproved = !approved.isEmpty() && !approved.equals("Unapproved");
    if (!isApproved) {
      listItem.add(HtmlUtil.makeSpanTag("unapproved", "[Draft]"));
    } else {
      String committed = page.getProperty(WikiPageProperty.COMMITTED);
      boolean isCommitted = committed.contains("true");
      if (!isCommitted) {
        listItem.add(HtmlUtil.makeSpanTag("uncommitted", "[To commit]"));
      }
    }
